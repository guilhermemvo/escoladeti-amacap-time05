var moduleProfessor = angular.module('escoladeti.professor', ['ngRoute','escoladeti.validacaoService','escoladeti.pessoaService']);
moduleProfessor.controller('ProfessorController', ['$scope', '$http', '$rootScope', '$routeParams', 'growl','validacaoService', 'pessoaService',function($scope, $http, $rootScope, $routeParams, growl,validacaoService, pessoaService) {

        $scope.message = 'Controller dos Professores!';
        $scope.page = {pageNumber: 0};
        $scope.pessoa = undefined;
        $scope.search = "";
        $scope.nome = "Professores";
        $scope.form = "professores";
        $scope.listaescola = [];
        $scope.caminholistar = "./View/Pessoa/Professor/listarProfessor.html";
        $scope.caminhoeditar = "./View/Pessoa/Professor/editarProfessor.html";
        $scope.caminhoinicial = "#/Pessoa/Professor/";
        $scope.caminhorelatorio = "professor/relatorio/listaRelatorio";
...
$scope.encontraEnderecoPorCEP = function (value) {
        ...
        $scope.incluir = function() {
            window.location.href = "#Pessoa/Professor/Novo/Editar";
        };
        $scope.init = function() {
            $scope.load(0);
        ...
        $scope.initEditar = function() {
            if ($routeParams.objeto === 'Novo') {
                ...
                $scope.professorDadoId($routeParams.objeto);
            }
            $scope.loadTodosEscola();
            $scope.loadTodosTipo();
            $scope.loadTodosPaises();
        ...
        $scope.loadTodosEscola = function() {
            $http({
                ...
                console.log(JSON.stringify(status));
                growl.error("Não foi possível listar as Escolas!");
            });
        };
        ...
        $scope.consultaCpf = function(cpf) {
            $http({
                method: "GET",
                url: "./professor/consulta/" + cpf,
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
            }).success(function(data, status) {
                if (data.cpf == $scope.pessoa.cpf) {
                    $scope.professores.cpf.$setValidity('cpfcadastrado', false);
                } else {
                    $scope.professores.cpf.$setValidity('cpfcadastrado', true);
                }
        ...
        $scope.remover = function(index) {
            if (index === '') {
                ...
                $scope.professor = $scope.page.list[index];
            }
            $rootScope.confirmaExclusao(excluir, 'nome');
        };
        $scope.removerServidor = function(professor) {
            $http({
                method: "DELETE",
                url: "./professor/remover",
                headers: {'Content-Type': 'application/json; charset=UTF-8'},
                data: JSON.stringify(angular.copy(professor))
            }).success(function(data, status) {
                growl.success("Cadastro excluído com sucesso!");
                $scope.load(0);
            }).error(function(data, status) {
                if (status == 500) {
                    growl.error("Não foi possível excluir o(a) Professor(a). O cadastro possui outras referências no sistema!");
                } else {
                    growl.error("Não foi possível excluir o(a) Professor(a)!");
                }
            });
        };
        ...
        $scope.load = function(numeroPagina) {
            $http({
                method: "GET",
                url: "./professor/listar/pag/" + numeroPagina,
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
            }).success(function(data, status) {
                $scope.page = formatPageList(data);
                console.log(JSON.stringify(data));
            }).error(function(data, status) {
                growl.error("Não foi possível listar os Professores!");
            });
        };
        ...
        $scope.salvar = function() {
            $http({
                ...
                window.location.href = "#Pessoa/Professor/";
                console.log(JSON.stringify(status));
        ...
        $scope.professorDadoId = function(id) {
            $http({
                method: "GET",
                url: "./professor/carregar/" + id,
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
            }).success(function(data, status) {
                $scope.pessoa = data;
            }).error(function(data, status) {
                $("<div></div>")
                        ...
                        .dialog({
                            autoOpen: true,
                            title: 'Erro ao carregar o professor ' + id,
                            width: 800,
                            height: 450,
                            modal: true
                        });
        ...
        $scope.procurarPorNome = function(nome) {
            if (nome != '') {
                $http({
                    method: "GET",
                    url: "./professor/procurarPorNome/" + nome,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
                }).success(function(data, status) {
                    $scope.page = data;
                }).error(function(data, status) {
                    growl.info("Não foi possível localizar o(a) Professor(a)!");
                });
            } else {
                $scope.load(0);
            }
