var moduleEscola = angular.module('escoladeti.escola', ['ngRoute', 'escoladeti.validacaoService','escoladeti.pessoaService']);

moduleEscola.controller('EscolaController', ['$scope', '$http', '$rootScope', '$routeParams','growl','validacaoService', 'pessoaService', function($scope, $http, $rootScope, $routeParams, growl,validacaoService, pessoaService) {

        $scope.message = 'Controller dos Escolas!';
        $scope.page = {pageNumber: 0};
        $scope.pessoa = undefined;
        $scope.search = "";
        $scope.nome = "Escolas";
        $scope.form = "Escolas";
        $scope.caminholistar = "./View/Pessoa/Escola/listarEscola.html";
        $scope.caminhoeditar = "./View/Pessoa/Escola/editarEscola.html";
        $scope.caminhoinicial = "#/Pessoa/Escola/";
        $scope.caminhorelatorio = "escola/relatorio/listaRelatorio";
...
$scope.encontraEnderecoPorCEP = function (value) {
        ...
        $scope.incluir = function() {
            window.location.href = "#Pessoa/Escola/Novo/Editar";
        };
        ...
        $scope.init = function() {
            ...
            $scope.loadTodosTipo();
            $scope.loadTodosPaises();
            //$scope.Escolas.razao.$setValidity('required', false);
            //$scope.Escolas.fantasia.$setValidity('required', true);
        };
        ...
        $scope.initEditar = function() {
            if ($routeParams.objeto === 'Novo') {
                ...
                $scope.escolaDadoId($routeParams.objeto);
        ...
        $scope.remover = function(index) {
            if (index === '') {
                ...
                $scope.escola = $scope.page.list[index];
            }
            $rootScope.confirmaExclusao(excluir, 'razao');
        };

        $scope.removerServidor = function(escola) {
            $http({
                method: "DELETE",
                url: "./escola/remover",
                headers: {'Content-Type': 'application/json; charset=UTF-8'},
                data: JSON.stringify(angular.copy(escola))
            }).success(function(data, status) {
                growl.success("Cadastro excluído com sucesso!");
                $scope.load(0);
            }).error(function(data, status) {
                if (status == 500) {
                   growl.error("Não foi possível excluir a Escola. O cadastro possui outras referências no sistema!");
                } else {
                   growl.error("Não foi possível excluir a Escola!");
                }
            });
        };
        $scope.consultaCnpj = function(cnpj) {
            ...
            $http({
                method: "GET",
                url: "./escola/consulta/" + cnpj,
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
            }).success(function(data, status) {
                if (data.cnpj == $scope.pessoa.cnpj) {
                    $scope.Escolas.cnpj.$setValidity('cnpjcadastrado', false);
                } else {
                    $scope.Escolas.cnpj.$setValidity('cnpjcadastrado', true);
                }
            }).error(function(data, status) {
                $scope.Escolas.cnpj.$setValidity('cnpjcadastrado', true);
            });
        ...
        $scope.load = function(numeroPagina) {
            $http({
                method: "GET",
                url: "./escola/listar/pag/" + numeroPagina,
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
            }).success(function(data, status) {
        ...
        $scope.salvar = function() {
            $http({
                ...
                window.location.href = "#Pessoa/Escola/";
        ...
        $scope.escolaDadoId = function(id) {
            $http({
                method: "GET",
                url: "./escola/carregar/" + id,
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
            }).success(function(data, status) {
                $scope.pessoa = data;
                console.log($scope.pessoa.enderecos);
            }).error(function(data, status) {
                $("<div></div>")
                        .css({overflow: "auto"})
                        .html(data)
        ...
        $scope.procurarPorNome = function(nome) {
            if(nome != ''){                                 
                $http({
                    method: "GET",
                    url: "./escola/procurarPorNome/" + nome,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
                }).success(function(data, status) {
                    $scope.page = data;
                }).error(function(data, status) {
                    growl.info("Não foi possível localizar a Escola!");
